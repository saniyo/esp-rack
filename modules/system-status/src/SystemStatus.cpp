...
void SystemStatus::buildIdentityForm(JsonObject& root) {
  ...
  // Chip model + IDF/Arduino SDK rev — physical-board identity, not
  // resource utilisation. First row (operator-preferred ordering — see
  // git history for the move from below-HW-Rev to top-of-tab).
  String chipSdk = String(ESP.getChipModel()) + " / " + String(ESP.getSdkVersion());
