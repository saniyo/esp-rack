...
// Post-login fallback URL. AuthenticatedRouting has a catch-all
// `<Navigate to={defaultPath}>` keyed off the manifest, so just
// landing the user on '/' lets the router pick the actual home.
// Kept as a helper (signature unchanged) so call sites don't have to
// know about the routing internals.
export const getDefaultRoute = (_features: Features) => '/';
